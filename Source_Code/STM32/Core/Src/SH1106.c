...
void SH1106_UpdateScreen(void) {
	...
	for (m = 0; m < 8; m++) {
		...
		column = SH1106_X_OFFSET;
		SH1106_WRITECOMMAND(0x00 | (column & 0x0F));
		SH1106_WRITECOMMAND(0x10 | ((column >> 4) & 0x0F));
...
void SH1106_Update_Range(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {

    if (x1 >= SH1106_WIDTH) return;
    if (x2 >= SH1106_WIDTH) x2 = SH1106_WIDTH - 1;
    if (y2 <= y1) return;

    uint16_t page_start = y1 / 8;
    uint16_t page_end   = (y2 - 1) / 8;   // 🔥 FIX

    for (uint16_t page = page_start; page <= page_end; page++) {

        SH1106_WRITECOMMAND(0xB0 + page);

        uint8_t col = x1 + SH1106_X_OFFSET;

        SH1106_WRITECOMMAND(0x00 | (col & 0x0F));
        SH1106_WRITECOMMAND(0x10 | (col >> 4));
        ...
        SH1106_I2C_WriteMulti(
            SH1106_I2C_ADDR,
            0x40,
            &SH1106_Buffer[page * SH1106_WIDTH + x1],
            width
        );
    }
}
...
void SH1106_Fill_Range(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, SH1106_COLOR_t color) {

    if (x1 >= SH1106_WIDTH) return;
    if (x2 >= SH1106_WIDTH) x2 = SH1106_WIDTH - 1;
    if (y2 <= y1) return;

    uint8_t fill = (color == SH1106_COLOR_BLACK) ? 0x00 : 0xFF;
    ...
    for (uint16_t page = page_start; page <= page_end; page++) {
        ...
        for (uint16_t x = x1; x <= x2; x++) {
            ...
            uint16_t index = x + page * SH1106_WIDTH;

            if (color == SH1106_COLOR_WHITE)
                SH1106_Buffer[index] |= mask;
            else
                SH1106_Buffer[index] &= ~mask;
        }
    }
...
char SH1106_Puts(const char* str, FontDef_t* Font, SH1106_COLOR_t color) {
	/* Write characters */
	while (*str) {
...
void SH1106_Clear_Range(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
	SH1106_Fill_Range(x1, y1, x2, y2, 0);
	SH1106_Update_Range(x1, y1, x2, y2);
}
void SH1106_ON(void) {
...
void SH1106_DrawBitmap(int x, int y,
                                   const uint8_t *bitmap,
                                   int w, int h,
                                   SH1106_COLOR_t color)
{
    int pages = h / 8;                // số page của bitmap
    int y_page = y / 8;               // page bắt đầu trên OLED
    int y_offset = y % 8;             // lệch bit trong page

    for (int col = 0; col < w; col++) {

        for (int page = 0; page < pages; page++) {

            uint8_t src = bitmap[page * w + col];

            int buf_index = (x + col) + (y_page + page) * SH1106_WIDTH;

            if (y_offset == 0) {
                // Fast path (align đúng page)
                if (color == SH1106_COLOR_WHITE)
                    SH1106_Buffer[buf_index] |= src;
                else
                    SH1106_Buffer[buf_index] &= ~src;
            } else {
                // Split byte (không align)
                uint8_t lower = src << y_offset;
                uint8_t upper = src >> (8 - y_offset);

                // ghi vào page hiện tại
                if (color == SH1106_COLOR_WHITE)
                    SH1106_Buffer[buf_index] |= lower;
                else
                    SH1106_Buffer[buf_index] &= ~lower;

                // ghi phần tràn sang page tiếp theo
                if ((y_page + page + 1) < (SH1106_HEIGHT / 8)) {
                    int next_index = (x + col) + (y_page + page + 1) * SH1106_WIDTH;

                    if (color == SH1106_COLOR_WHITE)
                        SH1106_Buffer[next_index] |= upper;
                    else
                        SH1106_Buffer[next_index] &= ~upper;
                }
            }
...
void SH1106_ShowFixedInfo(void) {
    SH1106_Clear();
    SH1106_DrawBitmap(WIFI_ICON_POSITION,wifi_off_icon_16x16, 16, 16, SH1106_COLOR_WHITE);
    SH1106_DrawBitmap(BELL_ICON_POSITION,bell_icon_16x16, 16, 16, SH1106_COLOR_WHITE);
    SH1106_DrawBitmap(TEMPERATURE_ICON_POSITION_X,TEMPERATURE_ICON_POSITION_Y,temperature_icon_16x16, 16, 16, SH1106_COLOR_WHITE);
    SH1106_DrawBitmap(HUMIDITY_ICON_POSITION_X,HUMIDITY_ICON_POSITION_Y,humidity_icon_16x16, 16, 16, SH1106_COLOR_WHITE);
    SH1106_DrawLine(0,18,127,18,SH1106_COLOR_WHITE);
    SH1106_DrawLine(0,48,127,48,SH1106_COLOR_WHITE);
    SH1106_UpdateScreen();
}
